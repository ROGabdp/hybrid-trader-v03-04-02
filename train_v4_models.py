...
"""
================================================================================
V4 模型訓練腳本 - 標準完整版本 (Standard)
================================================================================
本腳本執行完整的 V4 訓練流程：
1. 清除舊的特徵快取（強制用新的 30 次 MC Dropout 採樣重算）
2. 執行預訓練 (Pre-training)
3. 執行微調 (Fine-tuning) - 標準完整步數

與 V3 的差異：
- Buy Agent Fine-tune: 1,250,000 步 (標準)
- Sell Agent Fine-tune: 500,000 步 (標準)
...
def run_finetuning(train_buy=True, train_sell=True):
    ...
    print("=" * 60)
    print(f"  Buy Agent:  {FINETUNE_BUY_STEPS:,} steps (標準) {'✅' if train_buy else '⏭️ SKIP'}")
    print(f"  Sell Agent: {FINETUNE_SELL_STEPS:,} steps (標準) {'✅' if train_sell else '⏭️ SKIP'}")
    print(f"  Output:     {V4_MODELS_PATH}")
    print("=" * 60)
