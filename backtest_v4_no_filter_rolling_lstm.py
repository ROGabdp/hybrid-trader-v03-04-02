...
"""
================================================================================
V4 無濾網 Rolling LSTM 回測腳本 (No Filter Rolling LSTM Backtest)
================================================================================
在回測開始前重新訓練 LSTM 模型，訓練截止日期 = 回測開始日期 - 1 天。
這樣可以模擬真實交易情境：每次回測都使用只包含歷史資料的 LSTM 模型。

與 backtest_v4_no_filter.py 的差異：
- 在載入 LSTM 模型前，先重新訓練 T+1, T+5, T+20 模型
- 訓練資料截止日 = 回測開始日 - 1 天
- 強制清除特徵快取並重新計算
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='V4 Rolling LSTM 回測腳本 - 每次回測前重新訓練 LSTM',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
使用範例:
  python backtest_v4_rolling_lstm.py                           # 使用預設日期 (2023-01-01 至今)
  python backtest_v4_rolling_lstm.py --start 2022-01-01        # 從 2022-01-01 開始回測
  python backtest_v4_rolling_lstm.py --start 2023-06-01 --end 2024-06-30  # 指定完整日期範圍
        '''
    )
    parser.add_argument(
        '--start', 
    ...
    parser.add_argument(
        '--skip-lstm-train',
        action='store_true',
        help='跳過 LSTM 重訓 (使用現有模型，用於 debug)'
    )
...
# =============================================================================
# 無濾網回測器 (從 backtest_v4_no_filter.py 複製)
# =============================================================================
class NoFilterBacktester:
    ...
    def run(self, df: pd.DataFrame, feature_cols: list) -> dict:
        ...
        print("[Backtest] 模式: Rolling LSTM (無濾網)")
...
def main():
    ...
    print("=" * 60)
    print("V4 無濾網 Rolling LSTM 回測 (No Filter + Retrain LSTM)")
    print("=" * 60)
    print(f"  📅 回測開始日期: {args.start}")
    print(f"  📅 回測結束日期: {args.end if args.end else '最新資料'}")
    ...
    if not args.skip_lstm_train:
        if not train_lstm_models(train_end_date):
            print("[Error] LSTM 訓練失敗，無法繼續回測")
            sys.exit(1)
    else:
        print("\n[Skip] LSTM 重訓 (--skip-lstm-train)")
    ...
    # 計算剛剛訓練的模型的 train_start 和 train_end
    end_dt = datetime.strptime(train_end_date, '%Y-%m-%d')
    start_1d = (end_dt - timedelta(days=TRAIN_DAYS_1D)).strftime('%Y-%m-%d')
    start_5d = (end_dt - timedelta(days=TRAIN_DAYS_5D)).strftime('%Y-%m-%d')
    start_20d = (end_dt - timedelta(days=TRAIN_DAYS_20D)).strftime('%Y-%m-%d')
    
    # 直接載入剛剛訓練好的 LSTM 模型 (不使用 select_best_model，避免選到 R² 較高的舊模型)
