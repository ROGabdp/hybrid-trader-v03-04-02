...
"""
================================================================================
V4 DCA + AI Hybrid 有濾網 Fixed LSTM 回測
(With Filter + Fixed LSTM Backtest)
================================================================================
結合 DCA Hybrid 策略與固定 LSTM 模型功能。
使用固定的 LSTM 模型（後綴 _fixed），確保每次回測結果完全一致。
...
def get_fixed_lstm_paths(train_end_date: str) -> dict:
    """
    取得 Fixed LSTM 模型的預期路徑
    
    Args:
        train_end_date: 訓練截止日期
...
def check_fixed_lstm_exists(train_end_date: str) -> dict:
    """
    檢查是否存在符合條件的 Fixed LSTM 模型
    
    Returns:
        dict: {'all_exist': bool, 'paths': {模型路徑}, 'missing': [缺少的模型]}
    """
...
def train_and_save_fixed_lstm(train_end_date: str) -> bool:
    """
    訓練 LSTM 模型並儲存為 _fixed 版本
    """
    print("\n" + "=" * 60)
    print("📚 Step 0: 訓練並儲存 Fixed LSTM 模型")
    print("=" * 60)
    ...
    for model_name, script, start_date, save_dir in training_configs:
        ...
        try:
            ...
            print(f"[Training] ✅ {model_name} 訓練完成")
            ...
            if os.path.exists(src_pattern):
                ...
                print(f"[Fixed] 已儲存為: {os.path.basename(dst_path)}")
                
                # 複製相關的 scaler 和 meta 檔案
            ...
            print(f"[Error] {model_name} 訓練失敗: {e}")
            return False
...
def find_or_train_fixed_lstm(train_end_date: str) -> dict:
    ...
    Returns:
        dict: 包含模型路徑的資訊，用於後續載入
    """
    print("\n" + "=" * 60)
    print("🔍 Step 0: 尋找或訓練 Fixed LSTM 模型")
    print("=" * 60)
    ...
    if check_result['all_exist']:
        ...
        if train_and_save_fixed_lstm(train_end_date):
            ...
            return {'all_exist': False, 'paths': {}, 'missing': ['全部']}
...
def load_fixed_lstm_models(train_end_date: str, lstm_info: dict) -> bool:
    ...
    print(f"\n[LSTM] 載入 Fixed LSTM 模型 (train_end: {train_end_date})")
    ...
    for model_key, info in paths.items():
        ...
        if not os.path.exists(model_path):
            print(f"[Error] 找不到 {model_key}: {model_path}")
            return False
...
def main():
    ...
    print("=" * 70)
    print("V4 DCA + AI Hybrid 有濾網 Fixed LSTM 回測")
